# ASP.NET Web API, REST & HTTP — Q&A Study Guide

A progressive question-and-answer guide covering HTTP, REST, and ASP.NET Web API, from beginner to advanced.
...
**Beginner Level**
1. What is HTTP, and what are its core components (methods, status codes, headers)?
2. What is REST, and what are its guiding principles (constraints)?
3. What is ASP.NET Web API, and how do you build your first controller?
4. How does routing work? (Attribute routing, route parameters, constraints)
5. What is model binding and validation?

**Mid Level**
6. What is dependency injection and the service lifetimes (Singleton, Scoped, Transient)?
7. What is the middleware pipeline, and how do you write custom middleware?
8. What are filters? (Types, filter pipeline, vs. middleware)
9. What is content negotiation and how do you customize serialization?
10. How do authentication and authorization work? (JWT, `[Authorize]`, roles, policies)

**Advanced Level**
11. What are the API versioning strategies?
12. How does asynchronous programming work in Web API?

*(Guide in progress — more advanced questions to follow.)*
...
### Q: What is HTTP, and what are its core components (methods, status codes, headers)?

**A:** HTTP (HyperText Transfer Protocol) is a stateless, application-layer protocol used for communication between clients (like browsers or mobile apps) and servers over the web. "Stateless" means each request is independent — the server doesn't remember anything about previous requests unless you explicitly build in mechanisms like tokens or sessions.
...
|--------|------|------------|-------------|
| `GET` | ✅ | ✅ | Reads only; repeating changes nothing |
| `PUT` | ❌ | ✅ | Changes state, but repeating gives same final state |
| `DELETE` | ❌ | ✅ | Deletes (a change), but resource stays deleted on repeat |
| `POST` | ❌ | ❌ | Creates a new resource each time |
| `PATCH` | ❌ | ❌ (not guaranteed) | Changes state; may keep changing on repeat |
...
### Q: What is ASP.NET Web API, and how do you build your first controller?

**A:** ASP.NET Web API is a framework from Microsoft for building **HTTP-based services** (RESTful APIs) using C# and .NET. It's designed to return data (typically JSON) rather than HTML pages, making it ideal for serving browsers, mobile apps, IoT devices, and other services.
...
### Q: How does routing work? Explain attribute routing, route parameters, and constraints.

**A:** **Routing** maps an incoming HTTP request's URL to a specific controller and action method. In modern ASP.NET Web API, this is done almost entirely through **attribute routing**.
...
### Q: What is model binding and validation? How does request data become a C# object, and how do you validate it?

**A:** **Model binding** maps incoming HTTP request data (URL, query string, headers, body) into the parameters and objects of your action methods. **Validation** checks whether that bound data meets your rules before your logic runs.
...
### Q: What is dependency injection, and what are the service lifetimes (Singleton, Scoped, Transient)?

**A:** **Dependency Injection (DI)** is a design pattern where a class receives its dependencies from an external source rather than creating them itself. ASP.NET Core has DI **built into its core**. Getting service lifetimes wrong is one of the most common sources of subtle bugs.
...
### Q: What are filters? Explain the filter types, the filter pipeline, and how they differ from middleware.

**A:** **Filters** let you run code at specific stages *within* the MVC/Web API pipeline — before or after phases like authorization, model binding, action execution, and result formatting. Unlike middleware, filters have access to rich MVC context: the controller, action, its arguments, and its result.
...
### Q: What is content negotiation, and how do you customize serialization?

**A:** **Content negotiation** is how the server selects the best **representation format** for a response based on what the client asks for. One endpoint can serve the same data as JSON, XML, etc. This implements REST's "manipulation of resources through representations."
...
### Q: How do authentication and authorization work? Explain JWT, `[Authorize]`, roles, and policies.

**A:** **Authentication** answers *"Who are you?"* (verifies identity). **Authorization** answers *"What are you allowed to do?"* (decides access). They run in that order. REST APIs predominantly use the **JWT bearer token**.
...
### Q: What are the API versioning strategies, and how do you implement them?

**A:** **API versioning** manages changes to your API over time without breaking existing clients. Once consumers depend on your API, you can't freely change response shapes or behavior. Versioning lets you run multiple versions concurrently.
...
### Q: How does asynchronous programming work in Web API? Explain `async`/`await`, scalability, and pitfalls.

**A:** **Asynchronous programming** lets your API handle a request without blocking a thread while waiting for slow I/O (database, HTTP calls, files). Instead of a thread idling, it's released to serve *other* requests — dramatically improving **scalability**.
...
*End of guide (in progress). Next up: Advanced Question 13 — Global error handling and the ProblemDetails standard.*
